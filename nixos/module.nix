...
let
  ...
  # Bundled boiii server files archive
  boiiiServerFilesZip = ./boiii-server-files.zip;
  ...
  serverConfig = pkgs.writeText "server.cfg" ''
    ...
    ${optionalString (cfg.gameMode == "zm")
  ...
  launchScript = pkgs.writeShellScriptBin "bo3-server-launch" ''
    ...
    '' else ''
      # Select config filename based on gameMode
      case "${cfg.gameMode}" in
        zm)
          CONFIG_FILE="server_zm.cfg"
          ;;
        cp)
          CONFIG_FILE="server_cp.cfg"
          ;;
        *)
          CONFIG_FILE="server.cfg"
          ;;
      esac
      # Copy generated config to the appropriate config file
      cp -f ${serverConfig} "${serverDir}/zone/${CONFIG_FILE}"
      ARGS="$ARGS +exec ${CONFIG_FILE}"
    ''}
  ...
  installScript = pkgs.writeShellScriptBin "bo3-server-install" ''
    ...
    echo ""
    echo "=== Extracting BOIII server files to Wine prefix ==="
    if [ -f "${boiiiServerFilesZip}" ]; then
      BOIII_APPDATA="${cfg.dataDir}/.wine/drive_c/users/$(whoami)/AppData/Local/boiii"
      mkdir -p "$BOIII_APPDATA"
      unzip -o "${boiiiServerFilesZip}" -d "$BOIII_APPDATA"
      echo "Extracted BOIII server files"
    else
      echo "Warning: boiii-server-files.zip not found, skipping BOIII files extraction"
    fi
